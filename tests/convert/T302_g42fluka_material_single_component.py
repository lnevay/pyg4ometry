...
def Test(vis=False, interactive=False, fluka=True, outputPath=None, refFilePath=None):
    ...
    b1l = _g4.LogicalVolume(b1s, denseair, "b1l", reg)
    b2l = _g4.LogicalVolume(b2s, denseair, "b2l", reg)
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(outputPath / "T302_g42fluka_material_single_copmonent.gdml")

    # fluka conversion
    outputFile = outputPath / "T302_g42fluka_material_single_component.inp"
    if fluka:
    ...
    f.write(outputPath / "T302_g42fluka_material_single_component.flair")
